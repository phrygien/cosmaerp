...
new class extends Component
{
    ...
    {
        $this->facture = $facture->load(['commande.fournisseur', 'detailsFacture.detailCommande.product']);

        // Hydrate les champs depuis la facture existante
        $this->numero         = $facture->numero;
        $this->libelle        = $facture->libelle;
        $this->type           = $facture->type;
        $this->commande_id    = $facture->commande_id;
        $this->date_commande  = $facture->date_commande
            ? \Carbon\Carbon::parse($facture->date_commande)->format('Y-m-d')
            : '';
        $this->date_reception = $facture->date_reception
            ? \Carbon\Carbon::parse($facture->date_reception)->format('Y-m-d')
            : '';
        $this->remise         = $facture->remise;
        $this->tax            = $facture->tax;
        $this->state          = $facture->state;

        // Fournisseur déduit
        $this->fournisseurNom  = $facture->commande?->fournisseur?->name;
        $this->fournisseurCode = $facture->commande?->fournisseur?->code;

        // Hydrate les lignes depuis les détails existants
        $this->lignes = [];
        foreach ($facture->detailsFacture as $detail) {
            $this->lignes[] = [
                'detail_commande_id' => $detail->detail_commande_id,
                'designation'        => $detail->detailCommande?->product?->designation ?? '—',
                'quantite_commande'  => $detail->quantite_commande,
                'montant_HT'         => $detail->montant_HT,
                'taux_remise'        => $detail->detailCommande?->taux_remise ?? 0,
    ...
    // Inclut toujours la commande déjà liée à cette facture
    #[Computed]
    public function commandes()
    {
        return Commande::with('fournisseur')
            ...
            ->where(function ($q) {
                $q->whereDoesntHave('factures', fn($q) => $q->where('state', 0))
                    ->orWhere('id', $this->commande_id);
            })
    ...
    // ─── Watcher commande : déduire fournisseur + recharger lignes ─
    ...
    public function save(): void
    {
        ...
        try {
            ...
            // Supprime les anciens détails et recrée
            $this->facture->detailsFacture()->delete();

            foreach ($this->lignes as $ligne) {
                $this->facture->detailsFacture()->create([
            ...
            Flux::toast(
                heading: 'Facture mise à jour',
                text: "La facture \"{$this->facture->numero}\" a été modifiée avec succès.",
                variant: 'success'
            );
            ...
            Flux::toast(
                heading: 'Erreur',
                text: 'Impossible de modifier la facture : ' . $e->getMessage(),
                variant: 'danger'
            );
        }
    }
...
<div class="max-w-7xl mx-auto">
    ...
    <flux:breadcrumbs class="mb-5">
        <flux:breadcrumbs.item :href="route('facturation.list')">Factures</flux:breadcrumbs.item>
        <flux:breadcrumbs.item>{{ $facture->numero }}</flux:breadcrumbs.item>
    </flux:breadcrumbs>

    <div class="flex items-center justify-between mb-8">
        <flux:heading size="xl" level="1">Modifier la facture</flux:heading>

        <div class="flex items-center gap-2">
            <flux:button wire:click="resetForm">Annuler les modifications</flux:button>
            <flux:button :href="route('facturation.list')" variant="danger" wire:navigate>Retour à la liste</flux:button>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="lg:col-span-1 flex flex-col gap-6 lg:sticky lg:top-6">
            ...
            <flux:card class="p-0 overflow-hidden">
                ...
                <div class="mt-6 px-6 pb-6 border-t border-zinc-200 dark:border-zinc-700 pt-6 space-y-4">
            ...
            <flux:card class="p-0 overflow-hidden">
                ...
                <div class="mt-6 px-6 pb-6 border-t border-zinc-200 dark:border-zinc-700 pt-6">
    ...
    <div class="mt-8 flex items-center justify-end gap-x-3">
        <flux:button wire:click="resetForm">Annuler les modifications</flux:button>
        <flux:button wire:click="save" variant="primary">Enregistrer les modifications</flux:button>
    </div>
