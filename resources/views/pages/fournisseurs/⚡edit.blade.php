...
new class extends Component
{
    ...
    public function loadFournisseur(int $id): void
    {
        ...
        $this->code                = $f->code;
        $this->name                = $f->name;
        $this->raison_social       = $f->raison_social ?? '';
        $this->date_creation       = $f->date_creation ?? '';
        $this->state               = $f->state;
        $this->telephone           = $f->telephone ?? '';
        $this->fax                 = $f->fax ?? '';
        $this->mail                = $f->mail ?? '';
        $this->adresse_siege       = $f->adresse_siege ?? '';
        $this->code_postal         = $f->code_postal ?? '';
        $this->ville               = $f->ville ?? '';
        $this->adresse_retour      = $f->adresse_retour ?? '';
        $this->code_postal_retour  = $f->code_postal_retour ?? '';
        $this->ville_retour        = $f->ville_retour ?? '';
        $this->currentStep         = 1;

        $this->modal('edit-fournisseur')->show();
    }
    ...
    public function validateStep(): void
    {
        match ($this->currentStep) {
            1 => $this->validate([
                'code'          => 'required|string|max:50|unique:fournisseur,code,' . $this->fournisseurId,
    ...
    public function update(): void
    {
        ...
        Fournisseur::findOrFail($this->fournisseurId)->update([
            'code'               => $this->code,
            'name'               => $this->name,
        ...
        $this->modal('edit-fournisseur')->close();

        \Flux\Flux::toast(
            heading: 'Mise à jour du fournisseur',
...
<div>
    <flux:modal name="edit-fournisseur" class="md:w-2xl" :dismissible="false">
        <div class="space-y-5">

            <!-- Header -->
            <div>
                <flux:heading size="lg">Modifier le fournisseur</flux:heading>
                <flux:text class="mt-1">Modifiez les informations du fournisseur.</flux:text>
            </div>
            ...
            <nav aria-label="Progress">
                <ol role="list" class="flex overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700">
                    @foreach ($this->steps as $step => $info)
                        <li class="relative flex-1 overflow-hidden">
                            <div class="
                    flex items-center justify-center sm:justify-start gap-2 sm:gap-3 px-2 sm:px-4 py-3
                    @if ($step <= $currentStep) bg-white dark:bg-zinc-800
                    @else bg-zinc-50 dark:bg-zinc-900
                    @endif
                ...
                ">
                                ...
                                <div class="shrink-0">
                                    @if ($step < $currentStep)
                                        <span class="flex size-7 sm:size-8 items-center justify-center rounded-full bg-blue-600">
                                <flux:icon name="check" class="text-white" style="width:14px;height:14px;" />
                            </span>
                                    @elseif ($step === $currentStep)
                                        <span class="flex size-7 sm:size-8 items-center justify-center rounded-full border-2 border-blue-500">
                                <span class="text-xs sm:text-sm font-medium text-blue-600 dark:text-blue-400">
                                    {{ str_pad($step, 2, '0', STR_PAD_LEFT) }}
                                </span>
                            </span>
                                    @else
                                        <span class="flex size-7 sm:size-8 items-center justify-center rounded-full border-2 border-zinc-300 dark:border-zinc-600">
                                <span class="text-xs sm:text-sm font-medium text-zinc-400 dark:text-zinc-500">
                                    {{ str_pad($step, 2, '0', STR_PAD_LEFT) }}
                                </span>
                            </span>
                                    @endif
                                ...
                                <!-- Label : caché en mobile, visible sm+ -->
                                <div class="min-w-0 hidden sm:block">
                                    <p class="text-xs font-medium truncate
                            @if ($step < $currentStep) text-zinc-500 dark:text-zinc-400
                            @elseif ($step === $currentStep) text-zinc-900 dark:text-zinc-100
                        ...
                        ">
                                ...
                                <!-- Label mobile : uniquement step actif -->
                                @if ($step === $currentStep)
                                    <div class="min-w-0 block sm:hidden">
                    ...
                    @if ($step <= $currentStep) bg-blue-500
                ...
                <!-- Indicateur mobile : ex "Étape 2 sur 3" -->
                <p class="text-xs text-center text-zinc-400 dark:text-zinc-500 mt-2 sm:hidden">
            ...
            <div class="min-h-52">
                ...
                @if ($currentStep === 3)
                    <div class="space-y-5">
                        <div>
                            <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-3">Adresse siège</p>
                            <div class="space-y-3">
                        ...
                        <div class="border-t border-zinc-200 dark:border-zinc-700 pt-4">
                            <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-3">Adresse retour</p>
                            <div class="space-y-3">
            ...
            <!-- Actions -->
            <div class="flex items-center gap-2 pt-1 border-t border-zinc-200 dark:border-zinc-700">
                <flux:button
                    variant="ghost"
                    x-on:click="$flux.modal('edit-fournisseur').close()"
                >
                    Annuler
                </flux:button>
